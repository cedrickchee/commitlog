// Package freeport exposes functionality to get free (unused) network ports on
// your system.
package freeport
...
func GetWithErr(n int) (ports []int, err error) {
	...
	if n > blockSize-1 {
		return nil, fmt.Errorf("freeport: block size is too small for ports requested")
	}
...
func initialize() {
	if lowPort+maxBlocks*blockSize > maxPorts {
		panic("freeport: block size is too big or too many blocks requested")
	}
	...
	panic("freeport: can't allocated port block")
}
